...
### 제품 작업 중 하네스 자체 결함·개선점을 발견했을 때

진행 중인 제품 작업(코드/현재 `current.md`의 State) 도중 하네스 자체(`tools/harness/`, `docs/RULES.md` 등)의 문제나 개선점을 발견하더라도, 그 자리에서 즉시 하네스 코드를 고치지 않는다. 개선 후보로 기록만 해두고, 현재 진행 중인 제품 작업을 끝낸 뒤 별도 작업으로 수행한다 — 제품 작업의 변경 범위와 하네스 변경이 같은 커밋/리뷰 사이클에 섞이지 않도록 하기 위함이다.

단, 그 하네스 결함이 현재 제품 작업을 계속 진행할 수 없게 막는 경우(예: 잘못된 stale 판정으로 정상 작업이 계속 중단됨)에는 예외적으로 즉시 고칠 수 있다. 이때도 사용자 승인을 먼저 받고, 수정 내용은 제품 코드·`current.md`와 분리된 별도 커밋으로 만든다(제품 작업의 변경 파일에 하네스 파일이 섞이지 않게 한다).
